 ...
 * 飞行棋对决匹配房间
 */
public class AeroChessMatchRoom extends JFrame {
    ...
    // 活跃房间注册表
    private static final Map<Integer, AeroChessMatchRoom> activeRooms = new java.util.concurrent.ConcurrentHashMap<>();
    private static JFrame globalAeroChessHome = null;
    ...
    public static JFrame getGlobalAeroChessHome() { return globalAeroChessHome; }

    public AeroChessMatchRoom(String username, int userId, String mode, int maxPlayers,
        ...
        this.isReady = false;
        this.isCustomMode = mode != null && mode.startsWith("自定义");

        MessageCenter.startGlobalPush(username, userId);

        if (parentHome != null) {
            globalAeroChessHome = parentHome;
        }
        if (roomId > 0) {
            activeRooms.put(roomId, this);
        }

        setTitle("飞行棋对决 - " + mode + "模式");
    ...
    private void buildUI() {
        ...
        JLabel title = new JLabel("飞行棋对决 · " + (isCustomMode ? "自定义" : mode), JLabel.CENTER);
        ...
        actionBtn = new JButton(isCustomMode ? "添加机器人" : "匹配");
        actionBtn.setFont(new Font("Microsoft YaHei", Font.BOLD, 16));
        actionBtn.setForeground(Color.WHITE);
        actionBtn.setBackground(isCustomMode ? new Color(40, 140, 40) : ACCENT);
    ...
    private void sendDuelInvite(String friendName) {
        String inviteMsg = "DUEL_INVITE:" + roomId + ":" + mode + ":" + maxPlayers + ":飞行棋:" + System.currentTimeMillis();
        Thread t = new Thread(() -> {
    ...
    private void startMatchTimer() {
        if (matchCountdown <= 0) matchCountdown = 60;
        ...
        matchTimer = new javax.swing.Timer(1000, e -> {
            ...
            if (matchCountdown > 0) {
                ...
                stopMatchTimer();
                // 60秒到期：等待服务器 addBotsToRoom 推送 DUEL_STATE + DUEL_BOTS_JOINED
                // 不再本地填机器人/本地启动游戏，由服务器统一调度
                statusLabel.setText("匹配超时，正在补充机器人...");
            }
        });
        matchTimer.start();
    }
    ...
    private void addBotToRoom() {
        ...
        statusLabel.setText("正在添加机器人...");
        Thread t = new Thread(() -> {
            String resp = ServerClient.duelAddBot(roomId, username);
            SwingUtilities.invokeLater(() -> {
                if (resp.startsWith("SUCCESS")) {
                    statusLabel.setText("机器人已加入！");
                } else {
                    String msg = resp.contains("|") ? resp.split("\\|", 2)[1] : "添加失败";
    ...
    void applyRoomState(RoomState state) {
        ...
        if (!gamePageStarted && state.players.size() >= maxPlayers) {
            ...
            if (isCustomMode) {
                // 自定义模式：显示添加机器人按钮
                actionBtn.setText("添加机器人");
                actionBtn.setEnabled(true);
                actionBtn.setBackground(new Color(40, 140, 40));
            } else if (matching) {
                actionBtn.setText("取消匹配");
                actionBtn.setEnabled(true);
                actionBtn.setBackground(new Color(180, 120, 30));
    ...
    private void startGamePage() {
        ...
        try {
            ...
            System.out.println("[飞行棋匹配房间] 打开对决聊天失败: " + e.getMessage());
        }
    ...
    public static void receiveGameStart(int roomId, long seed, String mode) {
        AeroChessMatchRoom room = activeRooms.get(roomId);
    ...
    private void startGame(long seed, String mode) {
        ...
        for (int i = 0; i < maxPlayers; i++) {
            if (slotOccupied[i] && slotNameLabels[i] != null) {
                String name = slotNameLabels[i].getText();
                if (name != null && !name.isEmpty() && !allPlayers.contains(name)) {
                    allPlayers.add(name);
                }
            }
        }

        AeroChessGame game = new AeroChessGame(username, roomId, seed, mode, allPlayers, this);
    ...
    public void resetForNewGame() {
        ...
        gamePageStarted = false;
        stopMatchTimer();
        actionBtn.setText(isCustomMode ? "添加机器人" : "准备");
        actionBtn.setBackground(isCustomMode ? new Color(40, 140, 40) : ACCENT);
        actionBtn.setEnabled(true);
        statusLabel.setText("游戏结束，可再次准备");
        if (pollTimer == null || !pollTimer.isRunning()) startPoll();
        // 主动刷新房间状态（清除机器人残留、重置准备状态）
        refreshRoomState();
    }
    ...
    public static AeroChessMatchRoom getActiveRoom(int roomId) {
    ...
    public static void receiveMatchMove(int targetRoomId, String mode, int maxPlayers) {
        SwingUtilities.invokeLater(() -> {
            AeroChessMatchRoom oldRoom = null;
            for (AeroChessMatchRoom r : activeRooms.values()) {
    ...
    public static void joinRoom(String username, int userId, int roomId, String mode, int maxPlayers) {
        Thread t = new Thread(() -> {
            ...
            SwingUtilities.invokeLater(() -> {
                if (resp.startsWith("SUCCESS")) {
                    AeroChessMatchRoom room = new AeroChessMatchRoom(username, userId, mode, maxPlayers,
                            false, roomId, globalAeroChessHome);
