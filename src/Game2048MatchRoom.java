 ...
 * 2048 对决匹配房间
 */
public class Game2048MatchRoom extends JFrame {
    ...
    public Game2048MatchRoom(String username, int userId, String mode, int maxPlayers, boolean isCreator, int roomId, JFrame parentHome) {
        ...
        addWindowListener(new WindowAdapter() {
            public void windowClosing(WindowEvent e) {
                ...
                } else if (globalHome != null && globalHome.isDisplayable()) {
                    globalHome.setVisible(true);
                    globalHome.setLocationRelativeTo(null);
                } else {
    ...
    private void buildUI() {
        ...
        JLabel title = new JLabel("2048对决 · " + mode, JLabel.CENTER);
        ...
        for (int i = 0; i < 4; i++) {
            ...
            avatar.addMouseListener(new MouseAdapter() {
                public void mouseClicked(MouseEvent e) {
                    if (!slotOccupied[idx]) showFriendListForInvite(idx);
                }
        ...
        actionBtn.addActionListener(e -> {
            ...
            if (text.equals("匹配")) startMatching();
            else if (text.equals("取消匹配")) cancelMatching();
            else if (text.equals("准备") || text.equals("取消准备")) toggleReady();
        });
    ...
    private void sendDuelInvite(String friendName) {
        String inviteMsg = "DUEL_INVITE:" + roomId + ":" + mode + ":" + maxPlayers + ":2048:" + System.currentTimeMillis();
        Thread t = new Thread(() -> {
    ...
    private void updateSlotsVisibility() {
        ...
        for (int i = 0; i < maxPlayers; i++) slotsPanel.add(slotPanels[i]);
        ...
        Thread t = new Thread(() -> {
            ...
            SwingUtilities.invokeLater(() -> {
                if (resp.startsWith("SUCCESS")) {
                    ...
                    if (resp.contains("ALL_READY")) startGamePage();
                }
            });
        });
        t.setDaemon(true);
    ...
    public RoomState parseRoomState(String resp) {
    ...
    public void applyRoomState(RoomState state) {
        if (state.maxPlayers > 0) this.maxPlayers = state.maxPlayers;
        ...
        for (Map.Entry<String, Boolean> e : state.players.entrySet()) {
            if (idx < 4) {
                ...
                if (e.getKey().equals(username)) isReady = e.getValue();
            }
            idx++;
        }
        for (int i = idx; i < maxPlayers; i++) clearSlot(i);
        updateSlotsVisibility();
    ...
    public static void receiveGameStart(int roomId, long seed, String mode) {
        Game2048MatchRoom room = activeRooms.get(roomId);
    ...
    private void startGame(long seed, String mode) {
        ...
        setVisible(false);
        Game2048DuelGame game = new Game2048DuelGame(username, roomId, seed, mode, allPlayers, this, userId);
    ...
    private static final java.util.concurrent.ConcurrentHashMap<Integer, Game2048MatchRoom> activeRooms
            = new java.util.concurrent.ConcurrentHashMap<>();

    private static JFrame globalHome = null;

    public static void setGlobalHome(JFrame fh) { globalHome = fh; }
    public static JFrame getGlobalHome() { return globalHome; }

    public static Game2048MatchRoom getActiveRoom(int roomId) {
    ...
    public static void receiveMatchMove(int targetRoomId, String mode, int maxPlayers) {
        SwingUtilities.invokeLater(() -> {
            Game2048MatchRoom oldRoom = null;
            for (Game2048MatchRoom r : activeRooms.values()) {
    ...
    public static void joinRoom(String username, int userId, int roomId, String mode, int maxPlayers) {
        Thread t = new Thread(() -> {
            ...
            SwingUtilities.invokeLater(() -> {
                if (resp.startsWith("SUCCESS")) {
                    Game2048MatchRoom room = new Game2048MatchRoom(username, userId, mode, maxPlayers, false, roomId, globalHome);
